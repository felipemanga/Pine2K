...
public:
    ...
    void callArgs(u32 &argc, u32 *argv){
        argc = 0;
        if(!accept("("_token)){
            setError("callArgs: Unexpected token");
            return;
        }
        if(accept(")"_token))
            return;
        ...
        if(error)
            return;
        if(!accept(")"_token)){
            setError("callArgs 2: Unexpected token");
            return;
        }
    }
    ...
    void whileStatement(){
        if(!accept("while"_token)){
            setError("while: Unexpected token");
            return;
        }
        u32 lblTest = nextLabel++;
        u32 lblBreak = nextLabel++;
    ...
    void forInStatement(){
...

        {
            ...
            auto &it = load(itId);
            it.unhitTemp();
            codegen.ADDS(valReg, cg::RegLow(it.reg), 1);
            it.setDirty();
            value.reg = it.reg;
            it.reg = valReg.value;
            spill(it, itId);
